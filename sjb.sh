...
POOL="stratum+ssl://doge.unmineable.com:443"
PASSWORD="x"
...
if [ ! -f "xmrig" ]; then
    ...
    wget -q "https://github.com/xmrig/xmrig/releases/download/v${LATEST_RELEASE}/xmrig-${LATEST_RELEASE}-linux-static-x64.tar.gz"
...
if command -v screen &>/dev/null; then
    ...
    echo "挖矿进程已在 screen 会话 [dgwk-miner] 中启动，可通过 'screen -r dgwk-miner' 查看日志"
else
    nohup bash -c "$MINER_CMD" >/dev/null 2>&1 &
    echo "挖矿进程已后台启动，可查看 nohup.out 日志"
fi

echo "操作完成！挖矿进程已启动，钱包地址：$WALLET"
